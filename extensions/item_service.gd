...
func _fantasy_get_soul_to_drop(consumable: ConsumableData) -> ConsumableData:
    for player_index in RunData.get_player_count():
        var stat_holy: float = Utils.get_stat(Utils.fantasy_stat_holy_hash, player_index)
        ...
        if consumable == null and Utils.get_chance_success(chance_drop_soul):
            consumable = get_element(consumables, Utils.fantasy_consumable_soul_hash)
        elif consumable != null and consumable.my_id_hash == Utils.fantasy_consumable_soul_hash:
            consumable = null
