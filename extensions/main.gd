...
func _fantasy_holy_display() -> void:
    for i in range(_players.size()):
        ...
        if not UIHolyInstance.is_connected("mouse_entered", self, "fa_on_UIHoly_mouse_entered"):
            UIHolyInstance.connect("mouse_entered", self, "fa_on_UIHoly_mouse_entered", [Utils.get_stat("fantasy_stat_holy", i)])
...
func _fantasy_soul_display() -> void:
    for i in range(_players.size()):
        ...
        UISoulInstance.update_value(RunData.get_player_effect("fantasy_stat_soul", i))
        
        if not UISoulInstance.is_connected("mouse_entered", self, "fa_on_UISoul_mouse_entered"):
            UISoulInstance.connect("mouse_entered", self, "fa_on_UISoul_mouse_entered", [Utils.get_stat("fantasy_stat_soul", i)])
...
func _fantasy_holy_process() -> void:
    for i in range(_players.size()):
        ...
        is_instance_valid(UIHolyScenes[_players[i]]):
            UIHolyScenes[_players[i]].update_value(Utils.get_stat("fantasy_stat_holy", i))

func _fantasy_soul_process() -> void:
    for i in range(_players.size()):
        if _players[i] in UISoulScenes and \
        is_instance_valid(UISoulScenes[_players[i]]):
            UISoulScenes[_players[i]].update_value(Utils.get_stat("fantasy_stat_soul", i))
