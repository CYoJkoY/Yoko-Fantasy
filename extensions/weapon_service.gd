...
func init_melee_pet_stats(from_stats: MeleeWeaponStats, player_index: int, args := WeaponServiceInitStatsArgs.new()) -> MeleeWeaponStats:
    ...
    melee_pet_stats.cooldown = apply_attack_speed_mod_to_cooldown(melee_pet_stats.cooldown, Utils.get_stat(Utils.fantasy_pet_attack_speed_hash, player_index))
...
func init_ranged_pet_stats(from_stats: RangedWeaponStats, player_index: int, is_special_spawn := false, args := WeaponServiceInitStatsArgs.new()) -> RangedWeaponStats:
    ...
    ranged_pet_stats.cooldown = apply_attack_speed_mod_to_cooldown(ranged_pet_stats.cooldown, Utils.get_stat(Utils.fantasy_pet_attack_speed_hash, player_index))
...
func init_structure_pet_stats(from_stats: RangedWeaponStats, player_index: int, args := WeaponServiceInitStatsArgs.new()) -> RangedWeaponStats:
    ...
    structure_pet_stats.cooldown = apply_attack_speed_mod_to_cooldown(structure_pet_stats.cooldown, Utils.get_stat(Utils.fantasy_pet_attack_speed_hash, player_index))
