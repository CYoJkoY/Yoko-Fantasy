...
export(int) var offset_distance = 50
export(int) var spawn_cooldown = 30
export(String, FILE, "*.tscn") var child_projectile_path = "res://mods-unpacked/Yoko-Fantasy/content/projectiles/skeleton_archer_projectile/skeleton_archer_projectile_small.tscn"
...
func _ready():
    if !child_projectile_path: child_projectile_pool_id = Keys.generate_hash(child_projectile_path)
...
func _physics_process(delta) -> void:
    ...
    if _spawn_cooldwon < spawn_cooldown: return
...
func spawn_perpendicular_projectiles(index: int) -> void:
    ...
    if !is_instance_valid(new_projectile):
        new_projectile = load(child_projectile_path).instance()
    ...
    new_projectile.global_position = global_position + child_rotation * offset_distance
    new_projectile.rotation = child_rotation
    new_projectile.set_lifetime(free_time)
