...
func configure(start: Vector2, end: Vector2, fallback_direction: Vector2, progress: float, visibility: float, body_width: float, aura_width: float, core_width: float, trail_color: Color, secondary_color: Color, core_color: Color, ticks: float, phase: float, quality: int = 0) -> void:
	_quality = quality
...
func _draw() -> void:
	if _quality <= 1 and _aura_color.a > 0.0 and _has_drawable_area(_aura_polygon):
	...
	if _quality == 0 and _core_points.size() >= 2 and _core_color.a > 0.0:
