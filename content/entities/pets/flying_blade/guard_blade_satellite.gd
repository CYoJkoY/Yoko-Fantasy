extends Node2D
# Guard Blade Satellite — small orbiting sword. Orbits the player on a flat
# ellipse, distributed evenly via the shared coordinator's formation index.
# Trail is a top-level Line2D so world-space points don't drift with the
# satellite's own transform.
...
func _physics_process(delta: float) -> void:
    if not _initialized or not is_instance_valid(owner_pet) or owner_pet.dead:
        _disable_hitbox()
        visible = false
        return
    visible = true
    ...
    _state_ticks += ticks
    match _state:
        SatelliteState.ORBIT: _process_orbit(delta)
        SatelliteState.ATTACK: _process_attack(delta)
        SatelliteState.RETURN: _process_return(delta)
    _update_trail()

func _get_actual_orbit_radius() -> float:
    return orbit_radius + _formation_index * orbit_radius_offset
...
func _get_current_angle() -> float:
    ...
    return base + float(_formation_index) * (TAU / float(max(_formation_count, 1)))
...
func _process_orbit(delta: float) -> void:
    ...
    _velocity = (global_position - target_pos) / max(delta, 0.001)
...
func _process_attack(delta: float) -> void:
    if not _is_target_valid(_target):
        ...
        return
    var raw := min(_state_ticks / max(attack_ticks, 1.0), 1.0)
    var progress := FlyingBladeMotion.ease_out_cubic(raw)
    var prev := global_position
    global_position = FlyingBladeMotion.clamp_to_zone(FlyingBladeMotion.bezier2(_attack_start, _attack_control, _attack_end, progress))
    _velocity = (global_position - prev) / max(delta, 0.001)
    _attack_direction = prev.direction_to(global_position)
    if _attack_direction.length_squared() <= 0.1:
        _attack_direction = prev.direction_to(_target.global_position)
    if _attack_direction.length_squared() <= 0.1:
        _attack_direction = Vector2.RIGHT
    _body.rotation = lerp_angle(_body.rotation, _attack_direction.angle() - PI / 2, 0.3)
    var angle := _get_current_angle()
    _body.modulate.a = 0.4 + (0.5 + 0.3 * sin(angle * 2.0)) * 0.25
    _position_hitbox(prev, global_position)
    if raw >= 0.18 and not _hitbox_enabled:
        _enable_hitbox()
...
func _process_return(delta: float) -> void:
    ...
    var progress := FlyingBladeMotion.ease_out_cubic(min(_state_ticks / max(return_ticks, 1.0), 1.0))
    var prev := global_position
    global_position = FlyingBladeMotion.clamp_to_zone(FlyingBladeMotion.bezier2(_return_start, _return_control, _return_end, progress))
    _velocity = (global_position - prev) / max(delta, 0.001)
    var dir := prev.direction_to(global_position)
    if dir.length_squared() > 0.1:
        _body.rotation = lerp_angle(_body.rotation, dir.angle() - PI / 2, 0.2)
    if progress >= 1.0 or global_position.distance_squared_to(_return_end) <= 324.0:
...
func _begin_attack(target: Node2D) -> void:
    ...
    if _attack_direction.length_squared() <= 0.1:
        _attack_direction = Vector2.RIGHT
    var side := Vector2(-_attack_direction.y, _attack_direction.x) * (1.0 if randf() < 0.5 else -1.0)
    _attack_start = global_position
    _attack_end = FlyingBladeMotion.clamp_to_zone(target_pos + _attack_direction * attack_distance)
    _attack_control = FlyingBladeMotion.clamp_to_zone(
        _attack_start.linear_interpolate(_attack_end, 0.5) + side * 6.0
    )
    _disable_hitbox()
...
func _begin_return(apply_cooldown: bool = true) -> void:
    ...
    _trail_line.clear_points()
    if apply_cooldown:
        _cooldown = combat_coordinator.get_next_cooldown(attack_cooldown_ticks, ROLE) if combat_coordinator else rand_range(0.5, 1.5)
    else:
        _cooldown = 0.0
    _target = null
    _disable_hitbox()
    ...
    _return_end = _get_orbit_position()
    var dir := _return_start.direction_to(_return_end)
    if dir.length_squared() <= 0.1:
        dir = _velocity.normalized() if _velocity.length_squared() > 0.1 else Vector2.RIGHT
    var side := Vector2(-dir.y, dir.x)
    var dist := _return_start.distance_to(_return_end)
    _return_control = FlyingBladeMotion.clamp_to_zone(
        (_return_start + _return_end) * 0.5 
        + dir * dist * 0.15 
        + side * dist * 0.20
)
...
func _enqueue_coordinated_attack() -> void:
    if combat_coordinator:
        _attack_queued = combat_coordinator.enqueue_attack(self, ROLE)
...
func set_formation(index: int, count: int) -> void:
    _formation_index = clamp(index, 0, max(count - 1, 0)) as int
